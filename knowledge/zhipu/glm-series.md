...
**当前主推**: GLM-5.3（2026.08.14 发布，同基座后训练 Scaling，API 已上线 / 权重已开源）+ GLM-5.3-Flash（2026.08.26 发布并开源，系列首个原生多模态，¥0.8/¥2.8）+ GLM-5.2（2026.06 上线并开源，Solid 1M 上下文，专为长程任务而生）
**适用**: 企业合规部署、政企客户、本地化推理、中文编程、长程任务、1M 项目级上下文
**不适用**: 多模态场景需按模型区分（GLM-5.3/5.2/5.1 纯文本，需走 GLM-5V 等独立产品线；GLM-5.3-Flash 已原生多模态但输出仅文本）；GLM-5.3-Flash（¥0.8/¥2.8）已补齐极致低价位，其余型号价格仍高于 DeepSeek

## 当前主推模型

| 模型 | 定位 | 上下文 | 特点 | 推出时间 |
|------|------|--------|------|----------|
| 🚩 **GLM-5.3** | 最新旗舰（同基座后训练 Scaling） | 1M | 与 GLM-5.2 同基座（~743B MoE）；Terminal-Bench 3.0 28.3 开源第一，CyberGym 84.5%；始终思考模式（low/high/max）；API 已上线（08-19，百炼价与 5.2 持平）、权重已开源（08-28）、Coding Plan 已全量 | 2026.08.14 |
| **GLM-5.2** | 长程任务旗舰（在售） | 1M | Solid 1M 无损上下文，长程任务开源 SOTA，AA 综合榜 51 分前三，MIT 开源 | 2026.06 |
| **GLM-5.3-Flash** | 轻量多模态（在售） | 1M | 系列首个原生多模态，320B-A18B，稀疏+线性混合注意力，AA 57，MIT 开源，¥0.8/¥2.8 | 2026.08.26 |
...
- **模型**：GLM-5.3-Flash
- **公司**：智谱 AI
- **时间**：2026 年 8 月 26 日，发布并同步开源 [来源: 科创板日报 cls.cn/detail/2465814]
- **定位**：GLM-5 系列首个**原生多模态**模型（输入：文本/图像/视频/文件；输出：仅文本）[来源: docs.bigmodel.cn/cn/guide/models/vlm/glm-5.3-flash]
- **"原生多模态"定义**：①单模型端到端处理多模态输入，无需外挂视觉编码器或独立 VLM 产品线（GLM-5.3/5.2 看图需走 GLM-5V-Turbo/GLM-4.6V 独立产品线，官方 API 文档将 Flash 归入 vlm 目录）；②预训练阶段即混入 30T token 多模态语料从头训练，非文本基座后补视觉对齐（LLaVA 式）；③视觉能力内生于 Agent 循环——官方称模型可"自主判断何时需要'观察'，并利用视觉反馈指导下一步行动"（Visual Coding / BUA/CUA）。注意输出仅文本，即"原生多模态理解 + 文本输出"
- **与 GLM-5.3/5.2 的关系**：**并非同基座轻量版**——同基座的是 5.3/5.2（GLM-5/5.1/5.2/5.3 四代 ~743B 纯文本基座，仅改后训练）；Flash 是全新架构（320B-A18B、45 层 vs 92 层、稀疏+线性混合注意力），从多模态语料从头预训练，官方对标参数量级是 GLM-4.5（355B vs. 320B）而非 5.3 的缩水版。Ox-Alpha 盲测期分词器与 GLM-5.3 指纹一致仅说明同源分词器，不代表同基座 [来源: zhipuai.cn/zh/research/163]
- **架构**：总参数 320B、激活 18B（MoE，激活比 5.6%）、45 层（较 GLM-5.3 的 92 层近减半）；30T token 多模态语料预训练 [来源: zhipuai.cn/zh/research/163]
- **上下文**：1M / 最大输出 128K；thinking 仅支持 enabled（不可关闭）
- **注意力机制**：官方称**首个采用稀疏注意力与线性注意力混合架构的开源前沿模型**；较 GLM-5.3 注意力计算量降 3.01 倍、KV 缓存降 4.44 倍（按每 Head 每层归一化口径）；引入 IndexPool（索引器缓存向量加权池化，降低 1M 上下文下索引器时延/内存）与 mHC（流形约束超连接，提升 Scaling 能力）。官方自认短板：KV 缓存仍略高于 Kimi-K3 与 DeepSeek-V4-Flash [来源: 官方博客]
- **能力**：AA Intelligence Index **57**（同规模开源模型中位 29）；官方称与 Claude Opus 4.8 持平、高于 GLM-5.2 的 53 与 DeepSeek V4 Pro 的 53；Z.ai Code Bench 编程与 Opus 4.8 相当为官方自报口径，独立锚点应看 AA 分数 [来源: artificialanalysis.ai + 科创板日报]
- **定价**：每百万 token 输入 ¥0.8 / 输出 ¥2.8 / 缓存命中 ¥0.23；官方宣称"GLM-5.3 的 1/10（限时折扣 1/20）、Opus 4.8 的 1/40"（1/40 为官方混合口径营销数，按 AA 收录国际价 $0.15/$0.50 对 Opus $5/$25 逐项计算为输入 ~33 倍、输出 ~50 倍差）[来源: 科创板日报 + AA + 官方博客]
- **开源**：MIT 许可证，Hugging Face zai-org/GLM-5.3-Flash 与 -BF16 双仓库（HF 页面未直接核验，MIT 以 AA + 官方博客双源佐证）
- **营销打法**：发布前以匿名模型 **Ox-Alpha**（8/20 上线 OpenRouter，首日登顶创单日纪录、终结 DeepSeek 在 OpenCode 56 天霸榜、单日 62T tokens）在 OpenCode/OpenRouter 盲测；独立研究员经分词器指纹分析（25 组提示词与 GLM-5.3 计数完全一致、11 项探针全匹配）提前指向智谱，后被官方证实 [来源: 科创板日报]
- **算力与推理**：测试期与上线后流量由 **10 万张国产芯片**承载（自研高带宽互联；科创板日报援引相关人士称或为华为/海光/摩尔线程，未获官方确认）；SGLang 基座自研推理引擎 + EPD 分离 + W8A8/INT8/FP8/BF16 混合量化 + ReplaySSM + Layer Split，端到端较基线提升 3 倍，官方称单 token 成本已达主流 NVIDIA GPU 相当水平 [来源: 官方博客 + 科创板日报]
- **生态**：接入 ZCode、GLM Coding Plan（额度较 GLM-5.3 翻 3 倍、非高峰时段 5 折积分、每天 1 万张体验卡）；AA 收录 15 家 API 供应商转售 [来源: 官方文档 + AA]
- **短板（官方自认 + AA 实测）**：输出速度 41.7 t/s（同类中位 65，"Flash"指低成本而非快——混合注意力优化的是服务端显存/长上下文成本，解码速度另受国产芯片带宽制约）；TTFT 1.66s 尚可；冗长（AA 全程生成 150M tokens vs 中位 110M，部分抵消单价优势）；thinking 不可关闭（简单任务无法省思考开销）；AA Cost 维度仅 2/4 档（"reasonably priced"，卖点在单位智能价格而非绝对低价）[来源: AA]
- **成本三层归因（Why 定价能做到这么低）**：
  1. **架构层**：18B 激活 + 45 层 + 混合注意力（线性注意力管局部依赖、稀疏索引器管全局召回）直接攻击推理成本两大头（注意力 FLOPs 与 KV 缓存搬运）；本质是用训练算力换推理成本（30T 语料从头训练由规模化服务量摊销）；多模态原生单模型省掉 LLM+VLM 双栈服务成本
  2. **基础设施层**：国产芯片 + 软硬协同（以算力换带宽、以通信换显存应对国产芯片内存带宽瓶颈），绕开 NVIDIA 供货溢价
  3. **商业层**：MIT 开源使 API 定价受自部署套利约束、更贴近真实成本线而非纯补贴；Coding Plan 绑定 + 开源生态锁定转化习惯份额。详见 [开源权重下的 API 定价下限](../ai-general-notes/open-weights-api-pricing-floor.md)
...
- **定价**：百炼国际站（新加坡）$1.400 / $4.400 per 1M tokens（2026-09-02 核实）
...
| **开源 MIT 协议** | GLM-5.2 / GLM-5.1 / GLM-5.3（08-28 已开放）权重完全开放，支持商业部署 |
...
| 限制项 | 具体值 | 说明 |
|--------|--------|------|
| 纯文本（除 GLM-5.3-Flash 外） | GLM-5.3/5.2/5.1 不支持图像/视频/音频 | 多模态需使用 GLM-5V-Turbo / GLM-4.6V 等独立产品线；GLM-5.3-Flash 已原生多模态（输出仅文本） |
...
| 开源 + 商业可用 | GLM-5.2 / GLM-5.1 / GLM-5.3 | MIT 协议，可自部署（GLM-5.3 已于 08-28 开放） |
| 原生多模态输入 + 极致性价比 | GLM-5.3-Flash | 系列首个原生多模态（输出仅文本），¥0.8/¥2.8，AA 57 |
...
| 场景 | 原因 |
|------|------|
| 极致低价（除 Flash 外） | DeepSeek V4 价格更低（$0.27/$1.10 vs GLM-5.1 $0.98/$3.08）；GLM-5.3-Flash（¥0.8/¥2.8）已补齐低价位 |
| 多模态需求（除 Flash 外） | GLM-5.3/5.2/5.1 纯文本，需使用 GLM-5V-Turbo 等；GLM-5.3-Flash 已原生多模态（输出仅文本） |
...
- [GLM-5.3-Flash 官方研究博客](https://www.zhipuai.cn/zh/research/163)（2026-08-26）
- [GLM-5.3-Flash 官方文档](https://docs.bigmodel.cn/cn/guide/models/vlm/glm-5.3-flash)
- [GLM-5.3-Flash 英文技术博客](https://z.ai/blog/glm-5.3-flash)
- [科创板日报 - GLM-5.3-Flash 报道](https://www.cls.cn/detail/2465814)
