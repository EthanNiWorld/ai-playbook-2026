...
- **变体**：
  - `kimi/kimi-k2.7-code`（2026-06-15 百炼上线）：仅支持思考模式
  - `kimi/kimi-k2.7-code-highspeed`（2026-06-18 百炼上线）：功能完全一致，速度提升 5-6 倍 [来源: help.aliyun.com]
- **开源**：HuggingFace（moonshotai/Kimi-K2.7-Code），open-weight
- **场景差异 vs K2.6**：K2.7-Code 专注编码 Agent，不支持多模态视觉；K2.6 仍为通用旗舰（含视觉 + Agent Swarm）
...
| 思考模式 | 可开关的深度推理链（enable_thinking） |
...
**不支持**：联网搜索、thinking_budget 限制思考长度。

**参数默认值**：思考模式 temperature=1.0，非思考模式 temperature=0.6，top_p=0.95。
