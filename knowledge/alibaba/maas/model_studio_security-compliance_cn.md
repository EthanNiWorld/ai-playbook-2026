...
✅ 支持。百炼在虚拟化层提供安全防护，采用纵深防御 + 零信任架构。CMaaS（机密计算）方案通过 TEE 硬件隔离，天然免疫虚拟化逃逸和侧信道攻击——即使 hypervisor 被攻破，TEE 内的数据也无法被获取。
...
CMaaS 方案下，数据在 TEE（可信执行环境）内处理，即使操作系统或虚拟机管理器被攻破也无法窥探。任务结束后 TEE 环境销毁，内存数据物理上不可恢复。
...
百炼提供 CMaaS（Confidential MaaS）方案，通过 TEE 硬件隔离 + 远程证明 + 可重现构建，客户可以密码学方式验证数据是否真正"可用不可见"，实现数据保护从平台承诺到技术可验证的升级。
